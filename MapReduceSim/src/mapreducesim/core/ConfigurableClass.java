...
public abstract class ConfigurableClass {
	...
	/**
	 * Adds the given class to the map containing default instances of
	 * ConfigurabeClass subclasses.
	 * 
	 * 
	 * @param forClass
	 * @param defaultInstance
	 */
	public static <A extends ConfigurableClass> void addDefaultInstance(
			Class<A> forClass, A defaultInstance) {
	...
	public static <A extends ConfigurableClass> A getDefaultInstance(
			Class<A> forClass, XMLElement element) {
		try {
			return (A) defaults.get(forClass);
		} catch (Exception e) {
			throw new RuntimeException(
					"Failed to load a default instance for class: "
							+ forClass.getSimpleName(), e);
		}
	 ...
	 * configuration file given the desired subclass of configurable file, a
	 * default value, and the XMLElement to supply to the new class. This method
	 * should be used in cases where the node containing the configurable class
	 * is not a direct child of the root node - otherwise,
	 ...
	 *         or the default.
	 */
	public static <A extends ConfigurableClass> A instantiateFromXMLElement(
			Class<A> classType, A defaultInstance, XMLElement element) {
		A toReturn = null;
		try {
			if (element != null) {
				...
				toReturn = ReflectionUtil.attemptConstructorCallAndCast(
						classType, Class.forName(nameOfA), element);
				// if did not work, with no param
				if (toReturn == null)
					toReturn = ReflectionUtil.attemptConstructorCallAndCast(
							classType, Class.forName(nameOfA));
				if (toReturn == null) {
				...
				Msg.info("Null XMLElement for " + classType.getSimpleName()
						+ " given. Using default of "
	 ...
	 * desired subclass of ConfigurableFile. It is preffered to use
	 * instantiateFromSimConfig without a provided nameOfNode and write the XML
	 * config to have the name of the node be the name of the abstract class
	 * needed.
	 * 
	 * @param nameOfNode
	...
	public static <A extends ConfigurableClass> A instantiateFromSimConfig(
		...
		try {
			...
			if (element != null) {
				return instantiateFromXMLElement(classType, defaultInstance,
						element);
			} else {
	...
	/**
	 * Allows instantiation of a configurable class given just the name of the
	 * object wanted, e.g, calling it with FileSplitter.class will return a
	 * custom FileSplitter object if defined in the xml, or just the default if
	 * it wasn't specified in the config xml.
	 * 
	 * @param nameOfXMLNode
	...
	public static <A extends ConfigurableClass> A instantiateFromSimConfig(
		...
		return instantiateFromSimConfig(nameOfXMLNode, forClass,
				getDefaultInstance(forClass, null));
	}
}
