...
/**
 * The main class of the simulation. This is largely the same as SimGrid's
 * sample main class, but in addition to initialising SimGrid it also loads the
 * Simulation's own configuration XML file as the third argument.
 * 
 * The loaded configuration is static and is accessible everywhere within the
 * simulation through SimConfig. It is encouraged to access the configuration
 * from SimConfig through its helper methods.
 * 
 * Users are allowed to not provide a configuration XML file. In this case, the
 * configuration file is an XMLDocument with a null root. SimConfig accounts for
 * that possibility, and so should be used.
 * 
...
public class SimMain {
	...
	public static XMLDocument getPlatform() {
		return platform;
	}
	 ...
	 * 
	 * @param args
	 *            important parameters indicating the names/paths to XML files
	 *            for the platform, deployment, and optionally configuration XML
	 *            files for this simulation (in that order). If args is null,
	 *            default values are attempted. If args is less than 2 or
	 *            greater than 4, an input format message is printed and the
	 *            program exits.
	...
	public static void main(String[] args) throws NativeException {
		...
		if (args.length > 2) {
			...
			if (file.exists()) {
				...
				if (parse != null) {
					...
					System.out.print("** ERROR **\n" + "XML config file "
							+ args[2] + " not found or badly formatted:\n"
							+ file.read());
					System.exit(0);
				}
			} else {
		...
		// parse out the platform file
		SmartFile file = new SmartFile(args[0]);
		if (file.exists()) {
			XMLDocument parse = XMLDocument.parseDocument(file);
			if (parse != null) {
				platform = parse;
			...
			System.err.println("Platform file at " + file.getAbsolutePath()
					+ " does not exist.");
		}
