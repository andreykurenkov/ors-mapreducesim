...
public class SimulationsRunner {
	...
	/***
	 * Given XML documents, creates a temporary file with config content and runs a simulation, the log of which is written
	 * to logOut. The runSimulation that uses Files is called from this method once temporary files are created.At the end of
	 * the simulation, a row is added to this instance's set output file in CVS format.
	 * 
	...
	/**
	 * Given XML documents, creates a temporary file with config and runs a simulation, the log of which is written to
	 * logOut. The runSimulation that uses Files is called from this method once temporary files are created.At the end of
	 * the simulation, a row is added to this instance's set output file in CVS format.
	 * 
	 * @param platform
	...
	/***
	 * Given three XML documents, creates temporary files with their content and runs a simulation, the log of which is
	 * written to logOut. The runSimulation that uses Files is called from this method once temporary files are created.At
	 * the end of the simulation, a row is added to this instance's set output file in CVS format.
	 * 
	 * This method passes null for cvsValues, which leads to the default CVS values ("test number", "platform file",
	 * "deployment file", "configuration file") being recorded.
	 * 
